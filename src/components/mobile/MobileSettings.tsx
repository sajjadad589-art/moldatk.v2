    ...
    <div className="p-3.5 space-y-4 max-w-lg mx-auto pb-24">
      ...
      <div className="bg-white dark:bg-[#111c38] rounded-2xl p-4 border border-slate-200/90 dark:border-slate-800 shadow-xs space-y-3">
        ...
        <div className="grid grid-cols-3 gap-1.5 p-1 rounded-xl bg-slate-100 dark:bg-slate-900/80 border border-slate-200 dark:border-slate-800">
          <button onClick={() => onChangeViewMode('mobile')} className={(viewMode === 'mobile' ? 'bg-[#0B1F3B] text-white shadow-sm' : 'text-slate-600 dark:text-slate-400') + ' flex flex-col items-center justify-center p-2.5 rounded-lg text-xs font-bold transition-all'}>
            <Smartphone className="w-4 h-4 mb-1" />
            <span>هاتف 📱</span>
          </button>
          <button onClick={() => onChangeViewMode('desktop')} className={(viewMode === 'desktop' ? 'bg-[#0B1F3B] text-white shadow-sm' : 'text-slate-600 dark:text-slate-400') + ' flex flex-col items-center justify-center p-2.5 rounded-lg text-xs font-bold transition-all'}>
            <Monitor className="w-4 h-4 mb-1" />
            <span>حاسوب 🖥️</span>
          </button>
          <button onClick={() => onChangeViewMode('auto')} className={(viewMode === 'auto' ? 'bg-[#0B1F3B] text-white shadow-sm' : 'text-slate-600 dark:text-slate-400') + ' flex flex-col items-center justify-center p-2.5 rounded-lg text-xs font-bold transition-all'}>
      ...
      <div className="bg-white dark:bg-[#111c38] rounded-2xl p-4 border border-slate-200/90 dark:border-slate-800 shadow-xs space-y-3">
        ...
        <button onClick={onExportData} className="w-full flex items-center justify-center gap-2 rounded-xl bg-[#0B1F3B] text-white p-3 text-xs font-black">
