...
if (fs.existsSync(mobilePath)) {
  ...
  c = c.replace(/إعلان الإدارة/g, 'اعلانات').replace(/\.limit\(1\)/g, '.limit(10)').replace(/3500/g, '3000');
  write(mobilePath, c);
}
...
if (fs.existsSync(adminPath)) {
  ...
  if (!c.includes('type AdminAdSlide =')) {
    c = c.replace(/type AppNotification = \{[\s\S]*?\n\};/, m => `${m}\n\ntype AdminAdSlide = { id: string; title?: string | null; image_url: string; link_url?: string | null; sort_order?: number | null; is_active?: boolean | null; updated_at?: string | null; };`);
  }
...
console.log('Final ads manager OK: multi upload, red trash delete, link_url, 3s carousel, app notifications, owner-gated tab compatible.');
