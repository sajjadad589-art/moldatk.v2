...
function patchLogin() {
  if (!login.includes("cleanPass + 'moldatk'")) {
    const signInConst = `const { data, error } = await supabase.auth.signInWithPassword({\n          email: cleanInput,\n          password: cleanPass,\n        });`;
    const signInLet = `let { data, error } = await supabase.auth.signInWithPassword({\n          email: cleanInput,\n          password: cleanPass,\n        });`;
    if (login.includes(signInConst)) {
      login = login.replace(signInConst, signInLet);
      const firstFail = `        if (error || !data.user) {`;
      const retry = `        if ((error || !data.user) && cleanPass.length >= 4 && cleanPass.length < 6) {\n          const retry = await supabase.auth.signInWithPassword({\n            email: cleanInput,\n            password: cleanPass + 'moldatk',\n          });\n          data = retry.data;\n          error = retry.error;\n        }\n\n        if (error || !data.user) {`;
      login = login.replace(firstFail, retry);
      changed = true;
      console.log('patched: super admin mobile 4-digit login fallback');
    } else {
      console.warn('optional patch skipped: login fallback block not found');
    }
  }
  if (!login.includes("profile.role !== 'super_admin_manager'")) {
    ...
    if (login.includes(src)) {
      ...
      console.warn('optional patch skipped: manager role login block not found');
    }
  }
}
...
function patchOwnerPinUi() {
  if (!superAdmin.includes('const [ownerPinForm, setOwnerPinForm]')) {
    ...
    if (superAdmin.includes(src)) {
      superAdmin = superAdmin.replace(src, dst);
      changed = true;
      console.log('patched: owner pin state');
    } else {
      console.warn('optional patch skipped: managerForm state not found');
    }
  }
  ...
  if (!superAdmin.includes('const changeOwnerPin = async')) {
    ...
    if (superAdmin.includes(marker)) {
      superAdmin = superAdmin.replace(marker, insert + marker);
      changed = true;
      console.log('patched: owner pin handler');
    } else {
      console.warn('optional patch skipped: createManagerAccount marker not found');
    }
  }
  ...
  if (!superAdmin.includes('تغيير رمز حسابي')) {
    ...
    if (superAdmin.includes(src)) {
      ...
      console.warn('optional patch skipped: managerOpen form marker not found');
    }
  }
}
