...

{
  ...
  source = source.replace('          pricePerAmp: pricePerAmp > 0 ? formatCurrency(pricePerAmp) : \'\',', "          pricePerAmp: '',");
  source = source.replace('          remainingAmount: remainingAmount > 0 ? formatCurrency(remainingAmount) : \'\',', "          remainingAmount: '',");
  ...
  if (!source.includes('receipt-total') || !source.includes('المبلغ النهائي')) console.warn('Web receipt uses alternate final total markup; preserving upstream final amount.');
  write(path, source);
}

{
  ...
  let source = read(path);
  if (!source.includes('import android.graphics.drawable.Drawable;')) source = source.replace('import android.graphics.Typeface;\n', 'import android.graphics.Typeface;\nimport android.graphics.drawable.Drawable;\n');
  if (!source.includes('RECEIPT_LOGO_SIZE')) source = source.replace('    private static final int CONTENT_WIDTH = PAPER_WIDTH_PX - (PADDING * 2);', '    private static final int CONTENT_WIDTH = PAPER_WIDTH_PX - (PADDING * 2);\n    private static final int RECEIPT_LOGO_SIZE = 64;\n    private static final int RECEIPT_LOGO_GAP = 8;');
  source = source.replace('        String generatorName = val(r, "header", "المولدة");\n        lines.add(new DrawLine(generatorName, 31f, true, Layout.Alignment.ALIGN_CENTER, 10, true));', '        String generatorName = val(r, "header", "المولدة");\n        lines.add(new DrawLine("مولدتك", 31f, true, Layout.Alignment.ALIGN_CENTER, 5));\n        lines.add(new DrawLine(generatorName, 24f, true, Layout.Alignment.ALIGN_CENTER, 10, true));');
  ...
  if (!source.includes('"المبلغ النهائي\\n" + finalAmount')) console.warn('Native receipt uses alternate final amount wrapper; preserving upstream final amount.');
  write(path, source);
}

console.log('Minimal branded receipt normalization completed without blocking alternate receipt wrappers.');
