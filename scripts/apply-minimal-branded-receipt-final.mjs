...

{
  ...
  if (!source.includes('receipt-total') || !source.includes('المبلغ النهائي')) {
    console.warn('Minimal branded receipt: upstream web layout uses a different final-total wrapper; native SUNMI receipt remains authoritative.');
  }
  write(path, source);
}
...
console.log('Minimal branded receipt finalized for native SUNMI and normalized for web preview.');
