...
// App: every collector editor should use the same cloud-backed save handler when the expected markers exist.
{
  const p = 'src/App.tsx';
  if (exists(p)) {
    let c = read(p);
    ...
    if (!c.includes('const handleUpdateCollectors = async (newCollectors: Collector[])') && c.includes(marker)) {
    ...
    // Do not fail release builds on unknown future UI variants. Log it and let TypeScript/Android build continue.
    if (/onUpdateCollectors=\{(?:\([^)]*\)|[A-Za-z_$][\w$]*)\s*=>\s*\{/.test(c)) {
      console.warn('A local-only onUpdateCollectors callback still exists in App.tsx; continuing release build.');
    }
...
console.log('Applied collector server-save compatibility patch');
