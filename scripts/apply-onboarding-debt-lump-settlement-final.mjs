...
// -----------------------------------------------------------------------------
{
  ...
  const outstandingAnchorPresent = s.includes(outstandingAnchor) || /const outstanding = hasPricing \?/.test(s);
  if (!outstandingAnchorPresent) console.warn('skip: outstanding UI anchor already normalized');
  if (!s.includes('const visibleInvoices =') && s.includes(outstandingAnchor)) {
  ...
  const onboardingSubmitAlreadyApplied = s.includes("disabled={!subscriberToEdit && currentTierObj?.type !== 'free' && (!newDebtMode || (newDebtMode === 'prior' && !priorDebtMonthId))}")
  ...
  const advancedStart = s.indexOf('        {isAdvancedOpen && subscriberToEdit && (') >= 0
    ? s.indexOf('        {isAdvancedOpen && subscriberToEdit && (')
    : s.indexOf('        {hasPricing && isAdvancedOpen && subscriberToEdit && (');
  const confirmStart = advancedStart >= 0
    ? (s.indexOf('\n\n        {isConfirmUnpaidOpen', advancedStart) >= 0
      ? s.indexOf('\n\n        {isConfirmUnpaidOpen', advancedStart)
      : s.indexOf('\n\n        {hasPricing && isConfirmUnpaidOpen', advancedStart))
    : -1;
